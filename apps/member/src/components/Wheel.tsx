...
export default function Wheel({ segments, rotationDeg, spinning, spinMs }: Props) {
  const N = Math.max(segments.length, 1);
  const step = 360 / N;

  // warna bergantian (ringan + kontras cukup)
  ...
  const paths = useMemo(()=>{
    ...
    for (let i=0;i<N;i++){
      const start = (i*step -90) * Math.PI/180;    // -90 agar 0 derajat di atas
      ...
      const rotate = i*step + step/2; // utk tulisan
  ...
  return (
    <div className="wheel-frame">
      ...
      >
        <svg width="500" height="500" viewBox="0 0 500 500" style={{borderRadius:'50%'}}>
          {paths.map((p,idx)=>(
            <g key={idx}>
          ...
          {/* lingkaran tengah */}
          <circle cx="250" cy="250" r="60" fill="#0b1220" stroke="#1f2937" strokeWidth="4"/>
        </svg>
      </div>
    </div>
  );
